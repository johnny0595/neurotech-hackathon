...
def test_channel_config_commands_match_channel_2_jaw_setup():
    config = AppConfig()
    config.board.active_exg_channels = [2]
    config.board.rld_channels = [2]

    commands = channel_config_commands(config)

    assert "choff_1" in commands
    assert "chon_2_12" in commands
    assert "rldremove_1" in commands
